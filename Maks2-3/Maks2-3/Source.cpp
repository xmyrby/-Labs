...
void GetStudentsBin(Student* students, int& count)
{
	FILE* ft = NULL;
	if (fopen_s(&ft, "students.bin", "rt"))
	{
	...
	fread(students, sizeof(Student), count, ft);
	fclose(ft);
}
...
void SaveStudentsBin(Student* students, int& count)
{
	FILE* ft;
	if (fopen_s(&ft, "students.bin", "w+"))
	{
		printf("Ошибка чтения!");
		exit(1);
	}
	fwrite(&count, sizeof(int), 1, ft);

	fwrite(students, sizeof(Student), count, ft);
	fclose(ft);
}
...
int main()
{
	...
	SaveStudentsBin(students, count);
